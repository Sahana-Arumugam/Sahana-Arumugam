# Hi there, I'm Sahana Arumugam 

### Software Engineering @ VIT Vellore | AI/ML • Data Science • Full-Stack Development

I'm an Integrated M.Tech Software Engineering student passionate about building intelligent, scalable software that solves real-world problems. My interests span **Machine Learning, Distributed Systems, Cloud Computing, and Full-Stack Development**.

---

## Currently

*  AI/ML Intern at **Team Lambda** (DST-funded Government of India Project)
*  Exploring **Distributed Systems, MLOps, and Scalable Backend Engineering**
*  Building AI-powered applications and production-ready software

---

## 🛠 Tech Stack

**Languages**

`Python` • `Java` • `C++` • `C` • `JavaScript` • `TypeScript` • `R`

**Frontend**

`React` • `Next.js` • `Tailwind CSS` • `HTML5` • `CSS3`

**Backend**

`Node.js` • `Express.js` • `FastAPI` • `REST APIs`

**Machine Learning**

`TensorFlow` • `PyTorch` • `Scikit-learn` • `XGBoost`

**Cloud & DevOps**

`AWS` • `Docker` • `GitHub Actions` • `Azure`

**Databases**

`MongoDB` • `SQL` • `Firebase` • `Supabase`

---

##  Featured Projects

### 🏭 GlassForge Sentinel

AI-powered predictive maintenance platform integrating Machine Learning, RAG-powered LLMs, blockchain, and distributed telemetry processing for explainable industrial diagnostics.

🔗 **Repository:** [*GitHub Link*  ](https://github.com/Sahana-Arumugam/roller_failure_pred) 

---

### 🚦 MoVITex

Real-time campus crowd intelligence platform leveraging machine learning, event-driven processing, and smart routing to predict congestion across large-scale environments.

🔗 **Repository:** https://github.com/Sahana-Arumugam/movitex

---

### ⚙️ Bayesian Compiler Optimization Engine

Machine learning benchmarking framework utilizing Bayesian Optimization, automated hyperparameter tuning, and cross-validation to identify optimal predictive models.

🔗 **Repository:** *GitHub Link*

---

### 🌱 CarboLoom

Carbon footprint analytics platform featuring real-time emissions tracking, interactive dashboards, and sustainability insights for informed decision-making.

🔗 **Repository:** *GitHub Link*   |   🌐 **Live Demo:** *Portfolio Link*

---

## 🏆 Achievements

* 🥇 **1st Place** — Hack'HER'Thon 2026 National Hackathon
* 🥇 **1st Place** — IEEE TEMS Hack Expertise Hackathon
* 🏅 **Track Winner** — IEEE ITS Innovate for Impact Hackathon
* 🎖 **Proficiency Award** — Artificial Intelligence Excellence

---

## 🤝 Connect With Me

🌐 **Portfolio:** *Your Portfolio Link*

💼 **LinkedIn:** *Your LinkedIn Link*

📄 **Resume:** *Your Resume Link*

📧 **Email:** *Your Email Address*
